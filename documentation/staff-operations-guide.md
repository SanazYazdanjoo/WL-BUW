...
Coordinator `/staff/content` is labelled **Content**. Edit semester/content/WhatsApp only in the private Welcome Lounge workbook, then use **Preview content update** in the same screen, inspect all workbook changes/warnings, and publish only after review. The page offers a clean template when the workbook is missing, shows the last modified/published status and provides admin-only restore of earlier publications. Import operational MasterExcel data separately at `/staff/data`; official-source refresh is at `/staff/sources`; export the familiar workbook from `/staff/data`. The MasterExcel export does not contain full check-in, handover or audit state; back up the complete private data subtree. See the [workbook guide](content-workbook-guide.md), [semester handover](semester-handover.md), and [MasterExcel guide](master-excel-guide.md) for details.
