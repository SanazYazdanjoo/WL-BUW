...
Use Node 22.12+ and `npm ci`. Copy `.env.example` to `.env.local` and configure the Nextcloud host, root, account ID and app password. Never print/commit credentials or prefix them with `VITE_`. A browser login to Nextcloud does not authenticate the backend. `npm run dev` and `npm run preview` include the shared API. `npm start` serves the built `dist` plus API at 127.0.0.1:3000; HOST/PORT can be set server-side.

No credentials are needed to review labelled demo content. There is no automatic upload or migration of Nextcloud files. Follow [the workbook guide](content-workbook-guide.md) and [semester handover](semester-handover.md) to prepare the private source and publication structure. Keep all sensitive files outside `documents/` and public runtime content.
...
2. Set `NEXTCLOUD_BASE_URL`, `NEXTCLOUD_USERNAME`, `NEXTCLOUD_APP_PASSWORD`, and `NEXTCLOUD_ROOT_FOLDER` in Project Settings for Preview/Production as appropriate. Current values are `https://nextcloud.uni-weimar.de` and `/Welcome.Lounge_WiSe2026_27/APP`. `NEXTCLOUD_BROWSER_URL` is an optional staff navigation shortcut only. Do not put server settings in client variables or `vercel.json`.
...
- Refresh `/journey/<active-topic-id>`, `/events`, `/info`, `/help`, `/after-arrival/<active-topic-id>`, and an unknown URL; deep links work and unknown pages show not-found UI. After-arrival topics are listed directly in Info; there is no separate listing page.
...
Community RSS and verified resource maintenance is described in [community and support](community-and-support.md). Its normalized feed cache is private under `official-source-cache/community-rss.json`; it is separate from the authoritative source review queue. `npm run sources:discover-rss` inspects live RSS categories without changing the allowlist or saving content.
...
## Current Nextcloud configuration (temporary)

The current host is `https://nextcloud.uni-weimar.de` and root is `/Welcome.Lounge_WiSe2026_27/APP`. Configure `NEXTCLOUD_BASE_URL`, `NEXTCLOUD_ROOT_FOLDER`, `NEXTCLOUD_USERNAME`, and `NEXTCLOUD_APP_PASSWORD` in local server-only environment or the existing Vercel project settings. `NEXTCLOUD_BROWSER_URL` is an optional staff navigation shortcut only; its numeric Files UI ID is never used by backend storage. The application is allowed to access paths relative to the configured root, and no unrelated files are moved or exposed.
...
The six-sheet `Welcome-Lounge.xlsx` model supersedes the earlier preview/publish content workbook and the separate operational `staff-data/state.json` as the target for human-maintained data. See [the unified workbook guide](excel-database.md). The app checks the default workbook filename and keeps the previous system active while the file is missing. Upload a prepared workbook to the configured app-folder root to switch the app to the unified source; the staff setup screen does not create, import, or download it. Existing files are retained as migration/fallback sources.
