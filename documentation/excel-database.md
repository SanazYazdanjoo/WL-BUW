...
- **Tutors** (optional tab): ten rows, Tutor 1 … Tutor 10, each with a Name. Re-enter the names every semester; they are suggested in the Schedule grid and listed on the Statistics page. The Settings tab also holds **Schedule Start** / **Schedule End** (the days shown in the Schedule) and the shift times. Coordinators can edit all of these on Schedule → Semester setup. Statistics shows hours per tutor (worked up to today vs planned), calculated from the shift grid and the shift times.
