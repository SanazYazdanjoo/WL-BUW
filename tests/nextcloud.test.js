...
async function withApi(env, fetchImpl, run) {
  // Isolated streaming tests supply approval; production uses current content.
  const handler = nextcloudMiddleware(env, fetchImpl, async () => true);
...
test("missing credentials, traversal and writes never contact Nextcloud", async () => {
  await withApi(
    ...
    async (base) => {
      assert.equal((await fetch(`${base}/api/nextcloud/files`)).status, 404);
      assert.equal(
        (await fetch(`${base}/api/nextcloud/download?path=documents/file.pdf`))
          .status,
        503,
      );
      assert.equal(
        (await fetch(`${base}/api/nextcloud/files?path=..%2Fsecret`)).status,
        400,
      );
      assert.equal(
        (await fetch(`${base}/api/nextcloud/files`, { method: "POST" })).status,
        405,
      );
    },
  );
  ...
  await withApi(
    ...
    async (base) => {
      const response = await fetch(
        `${base}/api/nextcloud/download?path=documents%2Ffile.bin`,
      );
...
test("authentication errors are useful without exposing upstream responses", async () => {
  await withApi(
    ...
    async (base) => {
      const response = await fetch(
        `${base}/api/nextcloud/download?path=documents/file.pdf`,
      );
      assert.equal(response.status, 502);
      const body = await response.text();
      assert.match(body, /denied access/);
      assert.ok(
        !body.includes("private details") && !body.includes("test-secret"),
      );
    },
  );
});
