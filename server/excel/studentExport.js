...
export async function exportStudentsForDay(data, date) {
  ...
  const headers = ["Full name", "Matriculation number", "Country", "Study program", "Phone", "Email", "Enrolled", "Accommodation", "Contact (if no accommodation)", "City registration appointment", "Welcome materials", "Address", "Note", "Date added"];
  sheet.addRow(headers);
  sheet.getRow(2).font = { bold: true };
  for (const s of students)
    sheet.addRow([s.name, String(s.matriculationNumber || ""), s.country, s.studyProgram, s.phone, s.email, yesNo(s.enrolled), yesNo(s.accommodation), s.accommodationContact || "", yesNo(s.cityRegistration), yesNo(s.receivedBackpack), s.address, s.notes, s.legacyDate]);
